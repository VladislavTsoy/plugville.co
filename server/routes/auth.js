...
authRouter.post("/login", (req, res) => {
    if(req.body.username) {
        User.findOne({username: req.body.username}, (err, user) => {
            if(!user || err) return res.status(403).send({success: false, err: "Username or password is incorrect"});
            user.checkPassword(req.body.password, (err, match) => {
                if(err) return res.status(500).send({success: false, err});
                if(!match) return res.status(403).send({success: false, msg: "Username or password is incorrect."});
        ...
        return res.status(403).send({success: false, msg: "Username and password is incorrect"});
    };
});
